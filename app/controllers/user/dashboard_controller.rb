class User::DashboardController < ApplicationController
  ...
  def show
    ...
    @posts = Post.where(id: UserPost.where(user_id: FollowerFollowed.where(follower_id: current_user.id).pluck(:followed_id)).pluck(:post_id)).or(Post.where(id: UserPost.where(user_id: current_user.id))).order(id: :desc)
  end
end
